;((twod) => {
  ;(() => {
    "use strict"
    const { INVALID, GONE, MISMATCH, MOD_ERR, SYNTAX, DISALLOWED } = {
    ...
    class FileSystemHandle {
      ...
      async isSameEntry(param) {
        return (
          ...
          (this.kind === param.kind &&
            !!param[e] &&
            (await this[e].isSameEntry(param[e])))
        )
      }
    }
    ...
    class FileSystemDirectoryHandle extends FileSystemHandle {
      ...
      async getDirectoryHandle(e, t = {}) {
        ...
        return new FileSystemDirectoryHandle(
          await this[c].getDirectoryHandle(e, t)
        )
      }
      getDirectory(e, t = {}) {
      ...
      async getFileHandle(e, t = {}) {
        ...
        return (
          (t.create = !!t.create),
          new FileSystemFileHandle(await this[c].getFileHandle(e, t))
        )
      }
      getFile(e, t = {}) {
          ...
          if ("write" === e.type) {
            if (
              ...
              throw new DOMException(
                ...SYNTAX("write requires a data argument")
              )
            e = e.data
          } else {
            if ("seek" === e.type) {
    ...
    class FileGSAsync extends FileGS {
      ...
      async get() {
        ...
        if (
          cache?.lastModified &&
          response.lastModified === cache.lastModified
        ) {
          return cache
        } else {
          this.cache = response
    ...
    class FileWriterImpl {
      ...
      destroy() {
        eventBus && eventBus.emit("deleted", this),
          (this.deleted = true),
          (this.file = null)
      }
    }
    ...
    const promiseTimeoutImpl = function (e, t) {
      return t
        ? setTimeout.apply(this, [e, t])
        : (promiseTimeout.apply(this, [e]), 0)
    }
    const walk = (e, t) => {
      ...
      if ("array" == n) {
        const e = t,
          ...
          r = Object.assign(e, {
            ...
            addClass: (t) => (
              e.forEach((e) => e?.classList.add(t)), processor(e)
            ),
            removeClass: (t) => (
            ...
            children: (t) => {
              ...
              if (n)
                if (t) {
                  ...
                  return processor([].slice.call(n.children))
              return processor([])
            },
            find: (t) => {
              let n = []
            ...
            height: () => {
              const t = e[0]
              return (
                (t && (t === window ? window.innerHeight : t.offsetHeight)) || 0
              )
            },
            scrollTop: () => {
              const t = e[0]
            ...
            off: (t, n) => (
              t
                .split(" ")
                .forEach((t) => e.forEach((e) => e?.removeEventListener(t, n))),
              processor(e)
            ),
            trigger: (t, ...n) => (
              e.forEach((e) => {
    ...
    ;(async (self) => {
      ...
      let instance
      const eventManagerFactory = ({ sendPrefix, listenPrefix, cloneInto: n }) => {
        let r
        let suffix
        let s
        let o = 1
        ...
        const listenerCallback = (u_param) => {
			// result
          const {
            messageType,
            data,
            result,
          } = u_param instanceof CustomEvent
            ? u_param.detail
            : JSON.parse(u_param.attrName)
          if ("bridge.onpurge" == messageType)
		  	s !== window.document.documentElement && manager.refresh()
          else if ("unlock" == messageType) {
            c = false
            const e = l
            ;(l = []), e.forEach((e) => e())
          } else if ("message.response" == messageType) {
            if (null == data) throw "Invalid Message"
            ;((param1, param2) => {
              let n
            ...
            const s = data
              ? (data) => {
                  eventDispatch(`${sendPrefix}_${suffix}`, {
            ...
            r(
              {
                ...
                args: data,
                node:
                  u_param instanceof MutationEvent
                    ? u_param.relatedNode
                    : void 0,
              },
              s
        ...
        const f = (e) => {
          e && (suffix = e),
            suffix &&
              ...
              addEventListener(`${listenPrefix}_${suffix}`, listenerCallback, true))
        }
        let p = () => {}
        const manager = {
          init: async (token) => {
            ...
            isContentWorld
              ...
              : new Promise((e) => {
                  if (isContentWorld) throw "not supported"
                  {
                    ...
                    mutationInstance.observe(document, { childList: true })
                  }
                }).then(() => {
                  c = true
          ...
          refresh: () => {
            suffix && (manager.cleanup(), manager.init(suffix))
          },
          switchId: (e) => {
            suffix && manager.cleanup(), f(e)
          },
          send: (messageType, data, callback) => {
            let callbackFn, resultCallback
            "function" != typeof callback && null !== callback ? (callbackFn = callback) : (resultCallback = callback),
              isContentWorld && p()
            const u = () =>
              eventDispatch(`${sendPrefix}_${suffix}`, {
                messageType,
                data,
                result: resultCallback ? d(resultCallback) : null,
                n: callbackFn,
              })
            c ? l.push(u) : u()
          },
          sendToId: (suffix, messageType, data) => {
            eventDispatch(`${sendPrefix}_${suffix}`, {
              messageType,
              data,
              result: null,
          ...
          cleanup: () => {
            if (suffix) {
              removeEventListener(`${listenPrefix}_${suffix}`, listenerCallback, true)
              s = suffix = void 0
            }
          },
        }
        ...
        new Promise((reslove) => {
          ...
          const r = () => {
            e.send("userscripts", { action: "list" }, (event) => {
              !event || errorFn(event)
                ? setTimeout(r, Math.min((n *= 2), 5e3))
                : reslove(event.list)
            })
          }
          r()
        })
      const userscripts = await getUserScripts(eventManager)

      //   const userscripts = [
      // 	{
      // 		name: "file",
      // 		namespace: "测试",
      // 		path: "3a25f25c-b2d3-4016-8a89-62becd8999e4/source",
      // 		requires:[]
      // 	},
      // 	{
      // 		name: "file2",
      ...
      !(async function (fff, list, handler) {
        for (const { path, name, namespace, requires, storage } of list)
          [path, storage, ...requires]
            .filter((e) => e)
            .forEach((path) => {
              const [, flag, ...rest] = path.split("/")
              const namespaces = [
                slashReplacer(namespace || "<namespace missing>"),
                slashReplacer(name),
              ]
            ...
            const { value, lastModified } = await ((e, t, timeStamp) =>
              new Promise((resolve, reject) => {
                const timer = setTimeout(
                  () => reject(new DOMException(TIMED_OUT_ERROR)),
                  15e3
                )
                e.send(
                  "userscripts",
            ...
            await ((e, t, n, r) =>
              new Promise((resolve, reject) => {
                ...
                e.send(
                  ...
                  (e) => {
                    clearTimeout(o),
                      !e || e.error ? reject(e?.error) : resolve()
                  }
                )
              }))(eventManager, t, await n.text(), n.lastModified)
          }),
      })
      ...
      instance = await handleFn(registNameSpace, {
        name: "tc",
        eventEmitter,
        entries: namespaceInstance._entries,
      })
